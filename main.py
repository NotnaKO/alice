    ...
    if 'payload' in req['request']:
        if 'help' in req['request']['payload']:
            ...
            if user_id not in sessionStorage:
                ...
                return
            if 'first_name' not in sessionStorage[user_id]:
                res['response']['text'] += 'Пожалуйста, напиши своё имя'
                return
            ...
            if not sessionStorage[user_id]['game_started']:
                ...
                if len(res['response']['guessed_cities']) == len(cities.keys()):
                    ...
                    return
                elif 'attempt' in sessionStorage[user_id]:
                    if sessionStorage[user_id]['attempt'] == 1:
                        res['response']['text'] += 'Отгадай город с фотографии'
                        return
                    elif sessionStorage[user_id]['attempt'] == 2:
                        res['response'][
